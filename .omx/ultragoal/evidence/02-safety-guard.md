...
- Guard rejects normalized paths containing `/consume`, non-GET/HEAD methods, request bodies, unregistered endpoints, unsafe hosts, missing response schemas, and missing read-only review metadata.
...
- `src-tauri/src/connectors.rs` routes reset-credit connector requests through the guard before network execution.
